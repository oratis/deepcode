...
| `/mcp`                     | ✓           | ✓               | ✅                                                                                                                                                                                                |
| `/add-dir`                 | ✓           | ✓               | ✅ — persists to `permissions.additionalDirectories`; folded into the sandbox's `filesystem.allowWrite` by every host (CLI REPL/headless + app-server, so desktop/VS Code/LSP too)                |
...
| `/voice`                   | ✓           | ✓               | 🟡 — local whisper.cpp dictation on CLI (`/voice`: ffmpeg/sox → transcribe → pre-fill) **and** desktop (🎙 composer button, ffmpeg). Fully on-device; real-mic round-trip needs local verification |
| `/teleport`                | ✓           | ✗               | 🔄 M8                                                                                                                                                                                             |
| `/desktop`                 | ✓           | ✗               | 🔄 M6                                                                                                                                                                                             |
...
| `/plan`                    | ✓           | ✓               | ✅ — `/plan` enters plan mode, `/plan off` leaves; `/mode plan` still works                                                                                                                       |
